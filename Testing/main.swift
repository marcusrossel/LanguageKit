...
expressions.append(Expression("der Baum", in: german, group: word))
expressions.append(Expression("das Auto", in: german, group: word)) // 10
expressions.append(Expression("der PKW", in: german, group: word))
...
expressions.append(Expression("et tre", in: norwegian, group: word))
expressions.append(Expression("en bil", in: norwegian, group: word)) // 18
...
entries.append(Lexicon.Entry(title: expressions[5], translations: singleSynonyms[12]))
...
entries.append(Lexicon.Entry(title: expressions[12], translations: singleSynonyms[20]))
...
let result = lexicon.entries(origin: english, destination: norwegian)
